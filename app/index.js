...
export default function LoginScreen() {
  ...
  const getValueFunction = (dadosAluno) => {
    if (dadosAluno) {
      alunoLogado.setNome(dadosAluno.nome);
      alunoLogado.setEmail(dadosAluno.email);
      alunoLogado.setSenha(dadosAluno.senha);
      // Continue preenchendo os dados do aluno e da academia...
    }
  
    // Navegação se os dados forem válidos
    if (dadosAluno.email && dadosAluno.senha) {
      navigation.navigate('', { aluno: dadosAluno });
    }
  };
        ...
        <View style={style.areaLogin}>
          ...
          <View style={style.passwordContainer}>
            <TextInput
              ...
              value={password}
              style={[style.inputText, Estilo.corLight, { flex: 1 }]}
              onChangeText={(text) => setPassword(text)}
            />
            <Pressable onPress={() => setShowPassword(!showPassword)}>
              <FontAwesome5 name={showPassword ? "eye-slash" : "eye"} size={24} color="#0066FF" />
            </Pressable>
          </View>

          <Pressable onPress={handleLogin} style={[Estilo.corPrimaria, style.botao]}>
