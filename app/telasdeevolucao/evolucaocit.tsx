...
export default ({ route }) => {
  ...
  const [arrayDuracao, setArrayDuracao] = useState<Array<string>>([])
  ...
  useEffect(() => {
    const getPse = async () => {
      ...
      setArrayDuracao(newArrayDuracao)
    };
        ...
        {conexao ? carregandoDados ? (
          ...
          <View>
            ...
            <View style={[estilo.centralizado, { width: '90%', marginTop: 10 }]}>
              <BotaoSelect selecionado={!(arrayFiltrado[0].x === 0)} onChange={(value, index) => { handleSelectChange(value) }} titulo='Selecione um mês' max={1} options={arrayBotaoSelectSemRepeticoes}>
              </BotaoSelect>
            </View>
            ...
            <View style={{ marginLeft: '5%', marginBottom: '10%' }}>
              ...
              <RadioBotao
                ...
                selected={opcao}
                onChangeSelect={(opt, i: number) => { setOpcao(i); }}
              >
              </RadioBotao>
              ...
              {opcao == 0 ?
                <View>
                  <RadioBotao
                    ...
                    onChangeSelect={(opt, i: number) => { setOpcaoPeriodo(i); }}
                  >
                  </RadioBotao>
                  ...
                  <View style={{ flexDirection: 'row' }}>
                    ...
                    <View>
                      {arrayDuracao.map((item) => { return <Text style={[estilo.textoCorSecundaria, estilo.textoP16px]}>- Duração: {item} minutos</Text> })}
                    </View>
                  </View>
